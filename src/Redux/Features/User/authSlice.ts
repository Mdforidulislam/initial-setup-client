// Redux/Features/User/authSlice.ts
...
const authSlice = createSlice({
  ...
  initialState,
  reducers: {
    setAuth: (state, action: PayloadAction<{ token: string; userRole: string , user_Name: string}>) => {
      state.isAuthenticated = true;
      state.userRole = action.payload.userRole;
      state.user_Name = action.payload.user_Name;
      state.loading = false; 
      Cookies.set("accessToken", action.payload.token, { expires: 7, secure: true, sameSite: "Strict" });
    },
    clearAuth: (state) => {
